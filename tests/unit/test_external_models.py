...
def test_catalog_model_accepts_pascal_case_and_erorrs_typo() -> None:
    catalog = CatalogResponse.model_validate(load_json("catalog_success.json"))

    assert catalog.success is True
    assert catalog.total == 3
    assert catalog.errors == []
    assert [record.code_version for record in catalog.data] == ["843_1", "270_2", "270_3"]


def test_clinrec_model_accepts_obj_sections() -> None:
    response = ClinrecResponse.model_validate(load_json("clinrec_270_3.json"))
...
def test_nko_model_accepts_d_success_data_wrapper() -> None:
    response = NkoListResponse.model_validate(load_json("nko_list_success.json"))
