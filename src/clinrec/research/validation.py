...
def validate_corpus(corpus_root: Path) -> ValidationSummary:
    ...
    discovered_current = 0
    valid_current = 0
    invalid_current = 0
    discovered_previous = 0
    valid_previous = 0
    invalid_previous = 0
    orphan_current = 0
    orphan_previous = 0
    ...
    for kind, raw_path, current_code_version in iter_document_paths(corpus_root):
        ...
        if not manifest_path.exists():
            document_errors.append(issue(relative, "missing_manifest"))
        ...
        if not catalog_path.exists() and manifest.get("catalog_resolution_state") not in {
            "missing",
            "ambiguous_no_db_id_match",
            "ambiguous_multiple_db_id_matches",
        }:
            document_errors.append(issue(relative, "missing_catalog_record"))
        if manifest.get("sha256") != raw_sha:
            document_errors.append(issue(relative, "manifest_sha_mismatch"))
        if int(manifest.get("size") or -1) != len(raw_bytes):
            document_errors.append(issue(relative, "manifest_size_mismatch"))
        if manifest.get("validation") != "valid":
            document_errors.append(issue(relative, "manifest_not_valid"))
        if manifest.get("code_version") != code_version:
            document_errors.append(issue(relative, "manifest_code_version_mismatch"))
        info, validation_errors = minimal_validate_raw_document(
            raw_bytes,
            expected_code_version=code_version,
        )
        if info is None:
            document_errors.append(issue(relative, "invalid_raw_json", details=validation_errors))
        else:
            payload = load_payload(raw_path)
            if raw_path.parent.name != string_value(payload.get("id")):
                document_errors.append(issue(relative, "folder_name_not_json_id"))
            if string_value(payload.get("id")) != code_version:
                document_errors.append(issue(relative, "json_id_not_code_version"))
            if f"{info.code}_{info.version}" != code_version:
                document_errors.append(issue(relative, "code_version_fields_mismatch"))
            if kind == "current" and code_version not in selected_current:
                document_errors.append(issue(relative, "orphan_current_document"))
                orphan_current += 1
            if kind == "previous" and current_code_version is not None:
                ...
                if current_code_version not in selected_current:
                    document_errors.append(issue(relative, "orphan_previous_pair"))
                    orphan_previous += 1
            if manifest.get("db_id_state") == "mismatch":
                warnings.append(issue(relative, "catalog_db_id_mismatch"))
            if manifest.get("catalog_metadata_ambiguous"):
                warnings.append(issue(relative, "catalog_identity_ambiguity"))
            if manifest.get("catalog_resolution_state") == "missing":
                warnings.append(issue(relative, "catalog_metadata_missing"))
        ...
        if document_errors:
            ...
            if kind == "current":
                valid_current += 1
                if code_version in selected_current:
                    selected_valid_current.add(code_version)
            else:
                valid_previous += 1
    ...
    report = {
        ...
        "summary": {
            ...
            "selected_valid_current": len(selected_valid_current),
            "orphan_current": orphan_current,
            "discovered_previous": discovered_previous,
            "valid_previous": valid_previous,
            "invalid_previous": invalid_previous,
            "orphan_previous": orphan_previous,
            "raw_json_files": discovered_current + discovered_previous,
            ...
            "catalog_ambiguities": sum(
                1 for row in warnings if row["code"] == "catalog_identity_ambiguity"
            ),
...
def known_section_id(section_id: str) -> bool:
    return section_id in explicit_known_section_ids()


def explicit_known_section_ids() -> set[str]:
    return {
        "doc_title",
        "doc_whole",
        "section_intro",
